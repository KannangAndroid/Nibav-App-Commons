...
import com.nibav.camera.extensions.*
import com.nibav.camera.helpers.*
import com.nibav.camera.interfaces.GeoLocationListener
import com.nibav.camera.interfaces.MyPreview
import com.nibav.camera.models.CaptureMode
import com.nibav.camera.models.MediaOutput
import com.nibav.camera.models.MySize
import com.nibav.camera.models.ResolutionOption
import com.nibav.camera.services.GPSTracker
import com.nibav.commons.activities.BaseSimpleActivity
import com.nibav.commons.extensions.toast
import com.nibav.commons.helpers.PERMISSION_ACCESS_FINE_LOCATION
...
class CameraXPreview(
    ...
    private val employeeCode = empCode
    private var address = ""

    private val geoLocationListener = object : GeoLocationListener {
        override fun currentLocation(location: Location) {
            location?.let {
                currentLocation = location
                if (!gpsTracker?.getAddressLine(activity).isNullOrEmpty())
                    address = gpsTracker?.getAddressLine(activity) ?: ""
            }
        }
    }
    ...
    init {
        bindToLifeCycle()
        gpsTracker = GPSTracker(activity, geoLocationListener)
    }
                ...
                ensureBackgroundThread {
                    image.use {
                        if (mediaOutput is MediaOutput.BitmapOutput) {
                            ...
                            //if (mediaOutput is MediaOutput.OutputStreamMediaOutput) {
                            ...
                            ImageSaver.saveImage(
                                ...
                                onImageSaved = { savedUri ->
                                    //applyWaterMarkEffect(listener, savedUri, bitmap!!, 100, 100, Color.BLACK, 100, 60, false)
                                    activity.runOnUiThread {
                                        listener.onPhotoCaptureEnd()
    ...
    private fun applyWaterMarkEffect(
        ...
        /* val w = src.width
         ...
         canvas.drawText("EmpCode: 7153", x.toFloat(), src.height - y.toFloat(), paint)
         canvas.drawText(getCurrentDateTime(), x.toFloat(), y.toFloat() + size.toFloat() + 20f, paint)
         // Compress and save the bitmap to the file
         try {
             val imgFile = File(pathURI.path)

             FileOutputStream(imgFile).use { os ->
                 result.compress(Bitmap.CompressFormat.JPEG, 100, os)
                 os.flush()
                 os.close()
             }
             *//*  val exif = ExifInterface(imgFile.absolutePath)
              exif.setAttribute("UserComment", "testing for android development")

              exif.setAttribute(ExifInterface.TAG_GPS_LATITUDE,"ks dbkc")
              exif.setAttribute(ExifInterface.TAG_DATETIME,getCurrentDateTime())

              exif.setAttribute(ExifInterface.TAG_GPS_LONGITUDE,"jhsbf")
